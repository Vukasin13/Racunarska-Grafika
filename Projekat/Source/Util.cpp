...
unsigned loadImageToTexture(const char* filePath) {
    ...
    if (ImageData != NULL)
    {
        ...
        // --- OVO SU NOVE LINIJE KOJE NEDOSTAJU ---
        // Podesavamo kako se tekstura ponasa kad se zumira/umanjuje (GL_LINEAR je "smooth", GL_NEAREST je pikselizovano)
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // Filter za umanjivanje (BITNO!)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); // Filter za uvecavanje
        // ------------------------------------------

        glTexImage2D(GL_TEXTURE_2D, 0, InternalFormat, TextureWidth, TextureHeight, 0, InternalFormat, GL_UNSIGNED_BYTE, ImageData);

        // Opciono: Generisi mipmape za svaki slucaj (dobra praksa)
...
GLFWcursor* loadImageToCursor(const char* filePath) {
    ...
    if (ImageData != NULL)
    {
        ...
        int hotspotX = TextureWidth / 3;
        int hotspotY = TextureHeight / 3;
